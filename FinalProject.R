...
# To Extract the correlation coefficient
Corr$estimate
#0.2270356
...
#2)by spearman method
corr=cor(PWD$ADWG0021,PWD$ADWG2150 , use="complete.obs", method = 'spearman') 
Corr$estimate
#0.4427165

corr=cor(PWD$ADWG0021,PWD$ADWG0050, use="complete.obs",method="spearman") 
Corr$estimate
#0.4427165
#Both have Positive correlation 
...
#PVALUE= 0.7557 > 0.05  We don't have enough evidence to reject the null hypothesis and they are different
#which means that the mean of the 2 groups males and females is not different from each other, and also means that ADWG0021 is not different between male vs female.
...
#####shapiro test for males (Assuming males=2):
...
#####shapiro test for females(Assuming Females=1):
...
##Therefore we proved normality using qqplot, histogram and shapiro-test.

####checking homosc... : (i.e :Same variance)
boxplot(ADWG0021~Sex, data=PWD) #proving homoscedasticity using boxplot

var.test(ADWG0021~Sex, data=PWD) #p-value=0.4193 
#which is larger than the significant level alpha(0.05)  
#so we do not have enough evidence to reject the null hypothesis in
...
#we use Bartlett's test of homoscedasticity. In Bartlett's test the null
...
#__________________________Third point________________________________________
...
#install.packages("report") # Install the report package
#library(report) #load report package
install.packages("dunn.test") # Install the dunn.test package
library(dunn.test)
...
##since the result of levene test is p-value(0.8968) > alpha 
...
#the results of tukey test with adjusted p values with benferoni gives a p adjusted value 0.0437 between treatment A and B (A mixed with B) which is lower than 0.05 so we have evidence to reject null 
#so this leads to non normality of data(also then ADWG0021 is different in A with B after adjusting p value with benferoni)  , 
#while the p value of other treatments with each other is greater than 0.05 so we do not have evidence to reject the null so this leads to normality of other treatments(also ADWG0021 is not different between other treatment with each others(except A with B)) ,
#The mean of each pair of treatment is shown in the results.
